...
def create_augmented_set_bronze(train_set):
    ...
    pct=POS_PROPORTION
    pos_obs=train_set_positive.shape[0]
    obs_total=(pos_obs*100)/pct
...
def main():
    ...
    train_set=train_set.assign(silver_pos=np.where((train_set.cosine_similarity>=perc_99),1,0))
    ...
    train_set_final.to_csv(outputs_path+"sg_classifier/train_set_labeled_silver_sim_ds.csv",index=False)
    train_unlabeled.to_csv(outputs_path+"sg_classifier/unlabeled_set_silver_sim_ds.csv",index=False)
    train_labeled_unused.to_csv(outputs_path+"sg_classifier/weakly_labeled_unused_silver_sim_ds.csv",index=False)
